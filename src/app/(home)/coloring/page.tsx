...
export default function ColoringPage() {
  ...
  const floodFill = (ctx: CanvasRenderingContext2D, startX: number, startY: number, fillColorStr: string) => {
    ...
    const colorMatch = (pixelPos: number) => {
        ...
        if (a < 255 || (r < 20 && g < 20 && b < 20)) return false;
    ...
    <div dir="rtl" className="min-h-screen w-full bg-kids-bg text-white p-4 sm:p-6 md:p-8 flex flex-col items-center">
      <header className="w-full max-w-7xl flex items-center justify-between mb-6">
        <Link href="/dashboard" passHref>
          <Button variant="outline" className="utility-button bg-nile-dark text-gold-accent border-gold-accent/50 hover:bg-gold-accent hover:text-nile-dark">
            <Home className="w-5 h-5 ml-2" />
            الرئيسية
          </Button>
        </Link>
        <h1 className="text-3xl font-bold text-white font-display">لعبة ألوان الفراعنة</h1>
        <Button onClick={downloadImage} variant="outline" className="utility-button bg-nile-dark text-gold-accent border-gold-accent/50 hover:bg-gold-accent hover:text-nile-dark">
      ...
      <div className="flex flex-col lg:flex-row gap-8 w-full max-w-7xl h-[70vh]">
        ...
        <div className="flex flex-col items-center p-6 bg-nile-dark/30 rounded-2xl border-2 border-gold-accent/20 lg:max-w-xs">
          <h2 className="text-xl font-display text-gold-accent mb-4">لوحة الألوان</h2>
          ...
          <Button onClick={() => setSelectedColor(EraserColor)} variant="outline" className="utility-button w-full bg-nile-dark text-gold-accent border-gold-accent/50 hover:bg-gold-accent hover:text-nile-dark">
