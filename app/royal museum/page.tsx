...
/**
 * جلالة الملكة، تم إصلاح خطأ "Hydration Mismatch" الظاهر في الصورة.
 * الحل تم عبر تأجيل رندر المكونات التي تعتمد على المتصفح حتى يكتمل التحميل (Mounted).
 */
...
export default function App() {
  ...
  // التأكد من التحميل في جهة العميل فقط لمنع خطأ Hydration
  useEffect(() => {
    setHasMounted(true);
  }, []);
  ...
  const startRoyalNarration = (title, description) => {
    ...
    const fullText = `أهلاً بك في الأكاديمية الملكية.. حكاية "${title}" هي قصة من قلب التاريخ.. ${description}.. تذكر دائماً أن هذه القطعة ليست مجرد أثر، بل هي روح عظمة حضارة لا تموت.`;
  ...
  // إذا لم يتم التحميل بعد، لا تعرض شيئاً لمنع الخطأ
    ...
    <div className="relative w-full h-screen bg-black overflow-hidden select-none font-sans" dir="rtl">
        ...
        <div className="absolute inset-0 z-[100] flex items-center justify-center p-6 bg-black/40">
          <div className="royal-blur border-2 border-yellow-600 p-8 rounded-[3rem] max-w-lg w-full royal-style shadow-[0_0_80px_rgba(0,0,0,0.9)] animate-in fade-in zoom-in duration-500">
            <div className="flex justify-between items-center mb-8">
              ...
              <button onClick={() => {setActiveArtifact(null); window.speechSynthesis.cancel();}} className="text-zinc-500 hover:text-white transition-transform hover:rotate-90">
                <X size={35} />
              </button>
            </div>
            <h2 className="text-4xl font-black text-yellow-500 mb-6 border-r-4 border-yellow-600 pr-4">{activeArtifact.title}</h2>
            <p className="text-zinc-300 text-2xl leading-relaxed mb-10 italic">{activeArtifact.description}</p>
            <button 
              onClick={() => startRoyalNarration(activeArtifact.title, activeArtifact.description)}
              className="w-full bg-yellow-600 hover:bg-yellow-500 text-black font-black py-5 rounded-2xl flex items-center justify-center gap-4 text-xl transition-all active:scale-95"
            >
      ...
      {!isStarted && (
        <div className="absolute inset-0 z-[200] bg-black flex flex-col items-center justify-center text-center p-6 royal-style">
          <Crown size={120} className="text-yellow-600 mb-8 animate-pulse" />
          <h1 className="text-7xl md:text-9xl font-black text-yellow-500 mb-6 tracking-tighter">الأكاديمية الملكية</h1>
          <p className="text-2xl text-yellow-700/80 mb-16 font-bold tracking-[0.3em]">الإصدار الذهبي الخالي من الأخطاء</p>
          <button 
            onClick={() => setIsStarted(true)}
            className="bg-yellow-600 px-28 py-6 rounded-full text-4xl font-black text-black hover:bg-yellow-500 transition-all hover:scale-105 shadow-[0_20px_70px_rgba(212,175,55,0.4)]"
          >
            دخول المتحف
          </button>
        </div>
